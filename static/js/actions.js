...
$(document).ready(function () {
	...
	$(document).on('submit', 'form.ajax', function(e){	
		...
		var skip_empty_select_row = $this.hasClass('skip_empty_select_row');
		var allow_without_formset = $this.hasClass('allow_without_formset');
		
		var row_count = $this.find('tr.form_set_row').length;
		
		if(skip_empty_select_row){
			var er = 0;
			$this.find('tr.form_set_row').each(function(){
				$t = $(this);
				var value = $t.find('.check_empty select').val();
				if(!value){
					if(er == 0){
						$t.addClass('first');
					}
